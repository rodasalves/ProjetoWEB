...
$sql_consulta=sprintf("select nivel from registo;");
$res_consulta=mysqli_query($conn,$sql_consulta);
$num_consulta=mysqli_num_rows($res_consulta);
    
$nivel=$num_consulta['nivel'];
    
if (isset($_POST['bloquear'])){
    $sql_listax=sprintf("select * from registo where id=%d;",$_POST['id']);
    $res_listax=mysqli_query($conn,$sql_listax);
    $res_listax=mysqli_fetch_array($res_listax);
    if ($res_listax['nivel']==0){
        
        $sql_insere=sprintf("update registo set nivel=1 where id=%d;",$_POST['id']);
        
    } else{
        
        $sql_insere=sprintf("update registo set nivel=0 where id=%d;",$_POST['id']);
    }
    mysqli_query($conn,$sql_insere);
}
 $sql_lista=sprintf("select * from registo where nivel!=5 order by username;");
					...
					<div class="col">
						<div class="checkout_container d-flex flex-xxl-row flex-column align-items-start justify-content-start">
							...
							<div class="billing checkout_box">
								<div class="checkout_title">Utilizadores</div>
								<div class="checkout_form_container">
                                    ...
                                    <form method="post">
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo $reg_lista['username']; ?></td>
                                            <td><?php echo $reg_lista['pass']; ?></td>
                                            <td><?php echo $nivel; ?></td>
                   
                                            <td><input type="submit" name="bloquear" value="Bloquear/Desbloquear"></td>
