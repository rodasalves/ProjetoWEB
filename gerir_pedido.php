...
$sql_consulta=sprintf("select estado from pedido;");
$res_consulta=mysqli_query($conn,$sql_consulta);
$num_consulta=mysqli_num_rows($res_consulta);
    
$estado=$num_consulta['estado'];
    
if (isset($_POST['aceitar'])){
    $sql_listax=sprintf("select * from pedido where id=%d;",$_POST['id']);
    $res_listax=mysqli_query($conn,$sql_listax);
    $res_listax=mysqli_fetch_array($res_listax);
    if ($res_listax['estado']==0){
        
        $sql_insere=sprintf("update pedido set estado=1 where id=%d;",$_POST['id']);
        
    } else{
        
        $sql_insere=sprintf("update pedido set estado=0 where id=%d;",$_POST['id']);
    }
    mysqli_query($conn,$sql_insere);
}
 $sql_lista=sprintf("select * from pedido order by cliente;");
					...
					<div class="col">
						<div class="checkout_container d-flex flex-xxl-row flex-column align-items-start justify-content-start">
							...
							<div class="billing checkout_box">
								<div class="checkout_title">Pedidos</div>
								<div class="checkout_form_container">
                                    ...
                                    <form method="post">
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo $reg_lista['cliente']; ?></td>
                                            <td><?php echo $reg_lista['quantidade']; ?></td>
                                            <td><?php echo $reg_lista['nome_jogo']; ?></td>
                                            <td><?php echo $reg_lista['data']; ?></td>
                                            <td><?php echo $estado; ?></td>
                   
                                            <td><input type="submit" name="aceitar" value="Aceitar/Por Aceitar"></td>
